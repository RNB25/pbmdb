...
@if(session('gagal_creat'))
    <div id="toast-failed"
        class="fixed top-5 right-5 z-50 flex items-center w-full max-w-xs p-4 text-white bg-red-500 rounded-lg shadow"
        role="alert">
        <div
            class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-red-100 bg-red-600 rounded-lg">
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                <path d="M10 9V5a1 1 0 10-2 0v4a1 1 0 002 0zm0 4a1 1 0 100-2 1 1 0 000 2z" />
            </svg>
        </div>
        <div class="ms-3 text-sm font-normal">{{ session('gagal_creat') }}</div>
        <button type="button" class="ms-auto -mx-1.5 -my-1.5 text-white hover:text-black rounded-lg p-1.5 hover:bg-red-600"
            onclick="document.getElementById('toast-failed').remove()" aria-label="Close">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 14 14">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M1 1l6 6m0 0l6 6M7 7l6-6M7 7L1 13" />
            </svg>
